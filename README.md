...
- **VS Code extension**
  - `intellitest.backendUrl` (Settings): base URL of the IntelliTest Node server (for example `http://localhost:3000`). Required for **sign-in and generation** through the sidebar.
- **Stateful backend (`Server/`)**
  - See `Server/.env.example`: Mongo connection, `JWT_SECRET`, optional `JWT_EXPIRES_IN`, and LLM-related variables as documented there.
  - Guest access is optional on routes; the **extension** assumes you are authenticated and sends Bearer tokens once logged in.

- **Legacy / direct Groq (if used)**:
  - `GROQ_API_KEY` where applicable.
